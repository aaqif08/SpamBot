...
import { Badge, Button, Card, DemoBanner, EmptyState, ErrorState, PageHeader, Select, Skeleton, SourceTag, Spinner, Table, Tabs, Td, Th } from "@/components/ui";
...
export function ExplainabilityPage() {
  ...
  const [modelId, setModelId] = useState("active");
  const global = useApi(() => api.globalExplanation(modelId), [modelId]);
  const history = useApi(() => api.history({ page_size: 50, source: "" }), []);
  const [predictionId, setPredictionId] = useState("");
  const local = useApi(() => api.historyDetail(predictionId), [predictionId], predictionId !== "");
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Global: which features generally influence predictions (SHAP over a training sample). Local: why the model classified one specific account (SHAP waterfall + LIME)."
        actions={
      ...
      <section className="space-y-4">
        ...
        {global.error && !global.data && (global.status === 404 ? <EmptyState icon={Lightbulb} title="No trained model available" description="Run the training pipeline to populate SHAP results." action={<Link to="/training"><Button>Open training</Button></Link>} /> : <ErrorState message={global.error} onRetry={global.reload} />)}
        {global.data && g && (
          <>
            {global.data.is_demo && <DemoBanner compact text="This model was trained on synthetic demo data; the feature rankings describe the demo generator, not real social networks." />}
            <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
              ...
              <div className="space-y-4">
                <Card title="Importance by feature group" subtitle="Sum of mean |SHAP| per group (paper Table 4 groups)">
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Select value={predictionId} onChange={(e) => setPredictionId(e.target.value)} className="w-96" aria-label="Select a stored prediction">
            ...
            {history.data?.items.map((h) => <option key={h.id} value={h.id}>{h.account_identifier} · {h.prediction} · p={num(h.bot_probability, 2)} · {dateTime(h.created_at)}</option>)}
          </Select>
        </div>
        ...
        {predictionId && local.loading && !local.data && <Spinner label="Loading explanation (SHAP/LIME computed on demand for batch rows)…" />}
...
/** Batch rows are stored without explanations; fetch SHAP/LIME lazily via the explain endpoints. */
function LocalWithLazyExplain({ id, initial }: { id: string; initial: import("@/types/api").PredictResponse }) {
  ...
  const merged = useMemo(() => {
    const out = { ...initial };
    if (needShap && shap.data) out.shap_explanation = shap.data.explanation as import("@/types/api").ShapLocal;
    if (needLime && lime.data) out.lime_explanation = lime.data.explanation as import("@/types/api").LimeLocal;
